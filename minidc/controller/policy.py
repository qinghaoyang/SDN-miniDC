...
class AdaptivePolicy(object):
    ...
    def minUtilization(self):
        ...
        least_switch = next(iter(self.utilization))
        min_utilization = self.utilization[least_switch]
        for switch in self.utilization:
        	if self.utilization[switch] < min_utilization:
        		least_switch = switch
        		min_utilization = self.utilization[switch]
